import pf_data, file_decompressor, os
...
def read_prefetch_header(prefetch_file):
    ...
    if header[4:] == B'SCCA':
        ...
        get_prefetch_data(prefetch_file)

    elif header[0:3] == B'MAM':
        os.system('cls' if os.name == 'nt' else 'clear')

        file_decompressor.FileDecompressor(prefetch_file, prefetch_file[:-3] + "_decompressedFile.pf").decompress_file()
        get_prefetch_data(prefetch_file[:-3] + '_decompressedFile.pf')
...
def main():
    ...
    prefetch_file = input("Lütfen prefetch dosyanızın yolunu girin: ")

    if prefetch_file[0] == '"' and prefetch_file[-1] == '"':
        prefetch_file = prefetch_file[1:-1]

    read_prefetch_header(prefetch_file)
